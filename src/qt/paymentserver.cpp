...
const QString BITCOIN_IPC_PREFIX("zingcoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click zingcoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start zingcoin: click-to-pay handler");
    else
...
{
    // clicking on zingcoin: URLs creates FileOpen events on the Mac:
